...
public class CritX extends Module {
    ...
    public CritX() {
        super("CritX", "Auto Crit với nhiều chế độ", Category.COMBAT);
    }
    ...
    public void onTick() {
        ...
        switch (modeName) {
            case "Timing":
                ...
                break;
            case "Standard":
                if (autoJump.getValue() && player.isOnGround()) player.jump();
                if (!player.isOnGround() && player.getVelocity().y < 0) shouldCrit = true;
                break;
            case "Blatant":
                if (autoJump.getValue()) player.jump();
                if (player.fallDistance > 0.1 || player.getVelocity().y < 0) shouldCrit = true;
                else shouldCrit = true;
                break;
        }

        if (onlyOnGround.getValue() && player.isOnGround()) shouldCrit = false;
